...
const json=(status:number,body:unknown)=>new Response(JSON.stringify(body),{status,headers:{...cors,'Content-Type':'application/json; charset=utf-8','X-GCMBS-Api-Version':'10.0.73-canonical'}});
...
function readFieldSet(cat:any){const wf=new Set<string>((cat?.write_fields||[]).map(String));if(cat?.writable&&wf.size){for(const c of cat.columns||[])if(Number(c?.pk)>0)wf.add(String(c.name));for(const f of ['criado_em','atualizado_em'])if((cat.columns||[]).some((c:any)=>c.name===f))wf.add(f);return wf;}return null;}
function publicCatalog(cat:any,u:any,p:any[]){
 const wf=new Set<string>((cat.write_fields||[]).map(String)),heavy=new Set<string>((cat.heavy_fields||[]).map(String));
 const edit=!!cat.writable&&canFrom(u,p,String(cat.modulo||''),'EDICAO');
 let columns=Array.isArray(cat.columns)?cat.columns:[];
 if(cat.writable&&wf.size)columns=columns.filter((c:any)=>Number(c?.pk)>0||wf.has(String(c?.name||'')));
 columns=columns.filter((c:any)=>!TECH_HIDE.has(String(c?.name||''))&&!heavy.has(String(c?.name||'')));
 return {entity:cat.entity,modulo:cat.modulo,titulo:cat.titulo,writable:!!cat.writable,protected_write:!!cat.protected_write,aliases:cat.aliases||[],scope_fields:cat.scope_fields||[],columns,pk:cat.pk||[],can_edit:edit,communication_contract:'v73'};
}
...
async function entityList(req:Request,b:any){
 ...
 const heavy=new Set<string>((cat.heavy_fields||[]).map(String)),fields=readFieldSet(cat);rows=rows.map((r:any)=>({...r,data:selectedData(r.data,fields,heavy,false)}));
...
async function entityMutate(req:Request,b:any){
 const u=await auth(req);if(!u)return json(401,{message:'Sessão não autenticada ou expirada.'});const p=await permissions(Number(u.guarda_id)),cat=await catalogFor(b.entity);if(!cat)return json(404,{message:'Entidade online não encontrada.'});if(cat.protected_write)return json(409,{message:'Este módulo usa processamento protegido. Use o fluxo específico da tela.'});if(!cat.writable||!canFrom(u,p,cat.modulo,'EDICAO'))return json(403,{message:'Sem permissão de edição online para este módulo.'});
 const op=String(b.operation||'UPSERT').toUpperCase();if(!['UPSERT','DELETE'].includes(op))return json(400,{message:'Operação inválida.'});let key=norm(b.record_key);if(!key&&op==='UPSERT')key=`cloud:${randomUUID()}`;if(!key)return json(400,{message:'Registro inválido.'});
 ...
 const allowed=new Set<string>((cat.write_fields||[]).map(String));const incoming:any={};for(const [k,v] of Object.entries(b.data&&typeof b.data==='object'?b.data:{}))if(allowed.has(k))incoming[k]=v;
 const data=op==='UPSERT'&&existing?.data?{...existing.data,...incoming}:incoming;const scopeFields=Array.isArray(cat.scope_fields)?cat.scope_fields:[];let scope=[...new Set(scopeFields.map((f:string)=>Number(data[f])).filter((n:number)=>Number.isInteger(n)&&n>0))];
 ...
 const clientId=norm(b.client_change_id)||randomUUID();await db.begin(async sql=>{if(op==='DELETE')await sql`update public.mobile_entity_records set deleted=true,source='cloud',revision=revision+1,updated_by_guard_id=${Number(u.guarda_id)},updated_at=now() where entity=${cat.entity} and record_key=${key}`;else await sql`insert into public.mobile_entity_records(entity,record_key,modulo,scope_guard_ids,data,source,revision,deleted,updated_by_guard_id,updated_at) values(${cat.entity},${key},${cat.modulo},${scope},${sql.json(data)},'cloud',1,false,${Number(u.guarda_id)},now()) on conflict(entity,record_key) do update set scope_guard_ids=excluded.scope_guard_ids,data=excluded.data,source='cloud',revision=public.mobile_entity_records.revision+1,deleted=false,updated_by_guard_id=excluded.updated_by_guard_id,updated_at=now()`;await sql`insert into public.mobile_entity_changes(client_change_id,entity,record_key,modulo,operation,data,guarda_id,desktop_usuario_id) values(${clientId},${cat.entity},${key},${cat.modulo},${op},${sql.json(data)},${Number(u.guarda_id)},${Number(u.desktop_usuario_id)})`;});
 return json(200,{success:true,record_key:key,communication_contract:'v73'});
}
