...
function r18PatchSyncVersion(){
  ...
  if(e&&/10\.0\.(?:62|68|69)/.test(String(e.title||'')))e.title=String(e.title).replace(/10\.0\.(?:62|68|69)/g,'10.0.70');
}
async function r18AtualizarBadge(){
  ...
  try{
    ...
    const title=`Pendentes: ${Number(s.pendentes||0)} · Erros recentes: ${Number(s.erros_recentes||0)} · GCMBS Online/App 10.0.70`;
