...
// Folha: teto fixo 84h, prioridade 100%, arredondamento compatível com Desktop e OBS técnicas ocultas.
...
function hf7AjustarFolhaVisual(){
  ...
  if(max){max.value='84';max.disabled=true;max.setAttribute('readonly','');max.title='Limite institucional fixo de 84 horas';const label=max.closest('label');if(label){const no=[...label.childNodes].find(n=>n.nodeType===Node.TEXT_NODE&&String(n.nodeValue||'').trim());if(no&&!String(no.nodeValue).includes('(fixo)'))no.nodeValue='Limite máximo de horas (fixo)';}}
  document.querySelectorAll('#folhaV62Root .folha-list > div').forEach(el=>{
    const t=String(el.textContent||'').trim();
    if(/^OBS\.\s*[67]\b/i.test(t)||/Excedente do limite de 84h transferido/i.test(t)||/Ajustes decimais 50%/i.test(t))el.remove();
  });
}
...
if(!window.__gcmbsHF7Fetch){
  ...
  window.fetch=async function(input,init={}){
    ...
    let bodyTxt=typeof init?.body==='string'?init.body:'';
    ...
    if(action==='quadro_operacional'){
      r.clone().json().then(hf7AtualizarQuadro).catch(()=>{});
    }
    return r;
  };
}
