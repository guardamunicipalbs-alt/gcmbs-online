...
import {AuthenticatedProvider} from './data-provider.js?v=100085';
...
function renderBanco(){
  ...
  for(const x of b){
    ...
    if(String(x.classe)==='100')c100+=min;
    else c50+=min;
  }
...
async function boot(){
  ...
  window.addEventListener('gcmbs:v110-refresh',async()=>{try{await atualizarAoVivo();await carregarQuadro();window.dispatchEvent(new Event('gcmbs:v110-refreshed'));}catch(e){console.warn('[GCMBS V110] Falha ao recarregar dados apos sincronizacao:',e?.message||e);}});
...
if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw.js?v=100154',{updateViaCache:'none'}).catch(()=>{});}
