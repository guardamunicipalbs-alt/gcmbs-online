...
function r21PatchVersion(){
  ...
  if(onlineVersao&&onlineVersao.textContent!=='Online/App 10.0.76')onlineVersao.textContent='Online/App 10.0.76';
  const card=document.getElementById('appAtualizacaoCard');
  if(card&&/10\.0\.(?:62|68|69)/.test(card.innerHTML))card.innerHTML=card.innerHTML.replace(/10\.0\.(?:62|68|69)/g,'10.0.76');
  document.querySelectorAll('[data-app-version],.app-version,.online-version').forEach(el=>{
    if(/10\.0\.(?:62|68|69)/.test(el.textContent||''))el.textContent=(el.textContent||'').replace(/10\.0\.(?:62|68|69)/g,'10.0.76');
  });
  document.querySelectorAll('main *').forEach(el=>{
    if(el.children.length===0&&/^Online(?:\/App)?\s+10\.0\.(?:62|68|69)$/i.test(String(el.textContent||'').trim()))el.textContent='Online/App 10.0.76';
  });
}
...
console.info('[GCMBS] Paridade visual 10.0.76 ativa');
