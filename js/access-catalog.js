...
{id:'escala_extra_manual',nome:'Escala Extra',descricao:'Complementos e ajustes operacionais'},
...
{id:'manutencoes',nome:'Manutenção',descricao:'Manutenção de viaturas',mobile:'viaturas'},
{id:'abastecimentos',nome:'Abastecimento',descricao:'Histórico de consumo da frota',mobile:'viaturas'},
{id:'checklist_viaturas',nome:'Check-list de Viaturas',descricao:'Inspeção operacional da frota'},
{id:'relatorios_frota',nome:'Relatórios da Frota',descricao:'Viaturas, manutenções e abastecimentos',mobile:'viaturas'},
...
export function normalizarPerfil(session={}){const role=String(session.role||session.perfil||'').trim().toLowerCase(),cargo=String(session.cargo||'').trim().toUpperCase();if(role==='comandante'||(/\bCOMANDANTE\b/.test(cargo)&&!/SUBCOMANDANTE/.test(cargo)))return'comandante';if(role==='subcomandante'||/\bSUBCOMANDANTE\b/.test(cargo))return'subcomandante';return role||'gcm'}
export function controleTotal(session={}){return session.controle_total===true||['comandante','subcomandante'].includes(normalizarPerfil(session))}
