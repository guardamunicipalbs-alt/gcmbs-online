...
async function pmContexto(force=false){
  ...
  const [s,d,e,refs]=await Promise.all([pmCall('session'),pmCall('data'),pmCall('entity_list',{entity:'permutas',limit:500,offset:0}),pmCall('references')]);
...
function pmMirrorConfere(req,mirror){
  if(!req||!mirror||!req.desktop_referencia_id)return false;const q=req.payload||{},d=mirror.data||{};
  if(String(req.desktop_referencia_id)!==pmMirrorId(mirror))return false;
  const eq=(a,b)=>String(a??'')===String(b??'');
  return [[q.data,d.data],[String(q.turno||'').toUpperCase(),String(d.turno||'').toUpperCase()],[q.substituido_id,d.substituido_id],[q.substituto_id,d.substituto_id]].every(([a,b])=>a==null||a===''||eq(a,b));
}
...
function pmFase(st){
  ...
  if(['CANCELAMENTO_PENDENTE','CANCELAMENTO_PENDENTE_DESKTOP','CANCELAMENTO_COMANDO_PENDENTE'].includes(s))return 'Cancelamento registrado e aguardando sincronizacao.';
...
function pmRotuloModalidade(q={}){const m=String(q.modalidade||'ASSUNCAO').toUpperCase();return m==='TROCA_EXTRA'?'Troca bilateral de extras':m==='CESSAO_EXTRA'?'Assuncao de servico extra':'Assuncao de servico ordinario';}
...
function pmAjustarAssuncaoExtra(){
  ...
  if(m==='CESSAO_EXTRA'){
    ...
    const t=outroLab&&[...outroLab.childNodes].find(n=>n.nodeType===Node.TEXT_NODE&&String(n.nodeValue||'').trim());if(t)t.nodeValue='Servico extra do outro GCM que deseja assumir';
  }else if(m==='TROCA_EXTRA'){meuLab?.classList.remove('hidden');outroLab?.classList.remove('hidden');}
}

function pmRenderGestor(ctx){
  const host=pm$('listaPermutasSolicitadas');if(!host||!pmGestor(ctx.session))return;
  const comp=pmComp(),nomes=pmNomeMap(ctx.refs),mirrors=ctx.mirrors.filter(m=>pmCompMirror(m)===comp),reqs=ctx.requests.filter(x=>String(x.tipo||'').toUpperCase()==='PERMUTA'&&pmCompReq(x)===comp),pareados=new Set();
  for(const r of reqs)if(pmEncontrarMirror(r,mirrors))pareados.add(Number(r.id));
  const itens=[];
  ...
  for(const r of reqs.filter(x=>!pareados.has(Number(x.id)))){
    const q=r.payload||{},refTem=!!r.desktop_referencia_id,refExiste=refTem&&ctx.mirrors.some(m=>pmMirrorId(m)===String(r.desktop_referencia_id)),colisao=refTem&&refExiste&&!pmEncontrarMirror(r,ctx.mirrors),st=colisao?pmStatus(r.status_original||r.status):pmStatus(r.status),sol=r.nome_guerra||pmNome(nomes,r.guarda_id),sub=pmNome(nomes,q.substituido_id),fase=pmFase(st);
    const detalhe=colisao?'Referencia historica do Desktop nao corresponde a data/turno/participantes desta solicitacao. Historico preservado para auditoria.':(r.resposta||fase);
    itens.push({ord:String(q.data||r.created_at||''),html:`<article class="record-card" data-v62-pm-request="${pmEsc(r.id)}"><div class="record-card-head"><strong>${pmEsc(sol)} — solicitacao online</strong><span class="status-pill status-${pmEsc(st)}">${pmEsc(st)}</span></div><div class="record-meta">${pmFmt(q.data)} · Turno ${pmEsc(q.turno||'-')} · ${pmEsc(pmRotuloModalidade(q))}</div><div>${pmEsc(sub)} · ${pmEsc(fase)}</div>${detalhe!==fase?`<small>${pmEsc(detalhe)}</small>`:''}${colisao?'<div class="record-warning"><b>Auditoria:</b> esta solicitacao nao foi associada ao registro Desktop de mesmo numero porque os participantes nao conferem.</div>':''}</article>`});
  }
...
function pmCorrigirGestao(ctx){
  ...
  const reqs=ctx.requests.filter(x=>String(x.tipo||'').toUpperCase()==='PERMUTA'&&pmCompReq(x)===pmComp()),cards=[...host.querySelectorAll('.record-card')];if(cards.length!==reqs.length)return;
  reqs.forEach((r,i)=>{
    ...
    const card=cards[i],st=pmStatus(r.status_original||r.status);if(card.dataset.v62PmCollisionFixed===st)return;card.dataset.v62PmCollisionFixed=st;
    const pill=card.querySelector('.status-pill');if(pill&&pill.textContent!==st){pill.textContent=st;pill.className=`status-pill status-${st}`;}
    card.querySelectorAll('small').forEach(x=>x.remove());let w=card.querySelector('[data-v62-pm-collision]');if(!w){w=document.createElement('div');w.dataset.v62PmCollision='1';w.className='record-warning';card.appendChild(w);}w.innerHTML='<b>Auditoria:</b> referencia Desktop inconsistente; o status exibido foi restaurado ao estado proprio desta solicitacao.';
  });
}

async function pmAplicar(force=false){pmAjustarAssuncaoExtra();if(!pmAtivo()||pmBusy)return;pmBusy=true;try{const ctx=await pmContexto(force);if(pmAtivo()){pmRenderGestor(ctx);pmCorrigirGestao(ctx);pmAjustarAssuncaoExtra();}}catch(e){console.warn('[GCMBS] Permutas: correcao visual de auditoria nao aplicada:',e?.message||e);}finally{pmBusy=false;}}
...
function pmSetup(){
  pmAjustarAssuncaoExtra();pm$('pmModalidade')?.addEventListener('change',()=>setTimeout(()=>{pmAjustarAssuncaoExtra();pmSchedule(false);},0));pm$('pmCompetenciaFiltro')?.addEventListener('change',()=>pmSchedule(true));
  const h=pm$('listaPermutasSolicitadas');if(h)new MutationObserver(()=>pmSchedule(false)).observe(h,{childList:true,subtree:true});
  document.addEventListener('click',e=>{if(e.target.closest?.('[data-module="permutas"]'))setTimeout(()=>pmSchedule(true),80);},true);pmSchedule(false);
}
