import {MODULES,PRIMARY_ENTITY} from './communication-contract.js?v=100074';
...
const GENERIC_MODULES=new Set(MODULES.filter(m=>m.primaryEntity&&!m.dedicatedView).map(m=>m.id));
const PRIMARY=PRIMARY_ENTITY;
const EXPECTED=Object.fromEntries(MODULES.map(m=>[m.id,m.name]));
...
async function filePayload(id,prefix){const f=$(id)?.files?.[0];if(!f)return{};const b64=await new Promise((res,rej)=>{const rd=new FileReader();rd.onload=()=>res(String(rd.result||'').split(',')[1]||'');rd.onerror=()=>rej(new Error(`Não foi possível ler ${f.name}.`));rd.readAsDataURL(f);});return{[`${prefix}_nome`]:f.name,[`${prefix}_tipo`]:f.type,[`${prefix}_dados`]:`data:${f.type||'application/octet-stream'};base64,${b64}`};}
