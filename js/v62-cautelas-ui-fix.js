...
let caut62Busy=false,caut62RenderBusy=false,caut62FilterNeedsClear=false,caut62WasActive=false;
...
function caut62ClearInheritedFilter(){
  if(!caut62Ativo()||!caut62FilterNeedsClear)return;caut62FilterNeedsClear=false;const f=caut62$('onlineFiltro');if(f&&f.value){f.value='';f.dispatchEvent(new Event('input',{bubbles:true}));}
}
function caut62AjustarDescricoes(){
  const nova='Controle de equipamentos vinculados a GCMs ou viaturas.';const antiga='Cautelas individuais, de viatura e coletivas, com entrega, situação e devolução.';
  const desc=caut62$('onlineDescricao');if(desc)desc.textContent=nova;
  document.querySelectorAll('p').forEach(p=>{if(String(p.textContent||'').trim()===antiga)p.textContent=nova;});
}
function caut62NormalizarEditor(){
  ...
  caut62TextoLabel(caut62Label('patrimonio'),'Patrimônio/Série');caut62TextoLabel(caut62Label('data_entrega'),'Data de entrega');
  if(!caut62$('caut62ResponsabilidadeInfo')){
    const sec=guarda?.closest('.form-section')||guarda?.closest('section');if(sec){const p=document.createElement('p');p.id='caut62ResponsabilidadeInfo';p.className='muted';p.textContent='Vincule ao GCM e/ou à viatura conforme o equipamento. A modalidade de uso é Individual ou Uso comum / compartilhado, como no Desktop.';sec.querySelector('h3')?.insertAdjacentElement('afterend',p);}
  }
}
...
function caut62Run(){if(!caut62Ativo())return;caut62ClearInheritedFilter();caut62AjustarDescricoes();caut62NormalizarEditor();caut62AjustarAcoes();}
function caut62Install(){
  document.addEventListener('click',e=>{const nav=e.target.closest?.('#mainNav [data-module="cautelas"]');if(nav){caut62FilterNeedsClear=true;setTimeout(caut62Run,500);}const ret=e.target.closest?.('[data-caut62-return]');if(ret){e.preventDefault();e.stopImmediatePropagation();caut62Devolver(ret.dataset.caut62Return);return;}caut62Salvar(e);},true);
  const root=caut62$('appTela')||document.body;new MutationObserver(()=>{const ativo=caut62Ativo();if(ativo&&!caut62WasActive)caut62FilterNeedsClear=true;caut62WasActive=ativo;if(caut62RenderBusy||!ativo)return;caut62RenderBusy=true;requestAnimationFrame(()=>{caut62RenderBusy=false;caut62Run();});}).observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:['class','open','disabled','required']});
  caut62WasActive=caut62Ativo();if(caut62WasActive){caut62FilterNeedsClear=true;caut62Run();}
}
