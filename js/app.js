...
function temAcesso(modulo){
  ...
  if(modulo==='viaturas_menu') return provider.pode('abastecimento_viaturas') || provider.pode('manutencao_viaturas');
  if(modulo==='outros') return modulosOutros().length>0;
  return provider.pode(modulo);
}
function aplicarPermissoes(){
  const mapa={inicio:'dashboard',ocorrencias:'ocorrencias',eventos:'eventos_extra',escala:'escalas',viaturas:'viaturas_menu',permutas:'permutas',justificativas:'justificativas_faltas',banco:'banco_horas',acessos:'outros',avisos:'avisos',perfil:'perfil'};
  document.querySelectorAll('nav button').forEach(b=>{const mod=mapa[b.dataset.go];b.classList.toggle('hidden',b.dataset.go==='inicio'?false:!temAcesso(mod));});
}
...
async function boot(){
  ...
  document.querySelectorAll('nav button').forEach(b=>b.addEventListener('click',()=>{setView(b.dataset.go);if(b.dataset.go==='acessos')carregarOnlineCatalog().catch(()=>{});if(b.dataset.go==='inicio')carregarQuadro().catch(()=>{});if(b.dataset.go==='escala')renderEscalas()}));
...
if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw.js?v=100037',{updateViaCache:'none'}).catch(()=>{});}
