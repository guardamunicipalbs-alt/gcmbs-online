...
function gcmbsFreqRender(){
  ...
  const rowsHtml=list.map(r=>{const uid=gcmbsEsc(r.uid);const bloqueada=!!r.situacao_bloqueada||!gcmbsFreqCanEdit;const situacao=String(r.situacao||'PRESENTE').toUpperCase();return `<tr data-gcmbs-freq-row="${uid}">
    ...
    <td><select data-freq-field="situacao" ${bloqueada?'disabled':''} style="min-width:142px;min-height:38px;border:1px solid #cbd5e1;border-radius:8px;padding:0 8px"><option value="PRESENTE" ${situacao==='PRESENTE'?'selected':''}>PRESENTE</option><option value="FALTA" ${situacao==='FALTA'?'selected':''}>FALTA</option>${situacao==='FALTA JUSTIFICADA'?'<option value="FALTA JUSTIFICADA" selected>FALTA JUSTIFICADA</option>':''}</select></td>
