...
// Protege a lista filtrada contra sobrescrita da renderizacao legada.
...
let r17Cache=null,r17Busy=false,r17Timer=0,r17SectionObserver=null,r17ListObserver=null;
...
function r17Render(data){
  ...
  if(marker?.getAttribute('data-hf10-r17-list')===sig)return;
...
async function r17Refresh(force=false){
  ...
  catch(e){console.warn('[GCMBS] HF10 R17.2 Banco de Horas:',e?.message||e);}
...
function r17ObserveList(){
  const list=document.getElementById('listaBanco');if(!list||list.__gcmbsR17Observed)return;
  list.__gcmbsR17Observed=true;
  r17ListObserver=new MutationObserver(()=>{
    if(!r17Visivel()||!r17Gestor()||r17Busy||!r17Cache)return;
    const marker=list.querySelector('[data-hf10-r17-list]');
    if(!marker)r17Schedule(false,25);
  });
...
function r17Attach(){
  ...
  if(!r17SectionObserver){
    r17SectionObserver=new MutationObserver(()=>{if(r17Visivel()){r17RoleLayout();r17ObserveList();r17Schedule(true,80);}});
    r17SectionObserver.observe(s,{attributes:true,attributeFilter:['class']});
  }
  if(r17Visivel())r17Schedule(true,80);
}
document.addEventListener('change',e=>{
  if(e.target?.id==='bhGcmFiltro'){if(r17Cache)r17Render(r17Cache.data);r17Schedule(false,30);return;}
...
console.info('[GCMBS] HF10 R17.2 Banco de Horas por GCM + competencia ativo');
